...
const greekConfig: Record<keyof SelectedGreeks, GreekConfig> = {
  delta: {
    label: 'Delta',
    color: '#3b82f6',
...
export default function GreeksVisualization({ chartData, marketData }: GreeksVisualizationProps) {
  ...
  const toggleGreek = (greek: keyof SelectedGreeks) => {
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {(Object.entries(greekConfig) as [keyof SelectedGreeks, GreekConfig][]).map(([key, config]) => (
          <button
            key={key}
                            ...
                            <Fragment key={index}>
                              ...
                              >
                                {entry.value !== null && entry.value !== undefined
                                  ? (entry.value as number).toFixed(4)
                                  : 'N/A'}
                              </span>
                            </Fragment>
                          ))}
